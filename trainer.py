...
class Trainer(object):
    ...
    def train(self):
        ...
        self.criterionLatent = nn.MSELoss()
        self.criterionClass = nn.BCEWithLogitsLoss()
        # self.L1_loss = nn.L1Loss()
        self.criterionLin = nn.TripletMarginLoss()
        ...
        for step in range(start_step, total_steps):
            ...
            ########## forward
            self.fake_exp, self.real_raw_logit, self.real_raw_lin, self.real_exp_idt, self.real_exp_logit, self.real_exp_lin= self.G(self.real_raw, self.real_exp, self.args.mode)
            self.fake_exp_store = self.fake_exp_pool.query(self.fake_exp)
            # _, self.fake_exp_latent, _ = self.G(self.fake_exp)

            # self.fake_exp2, self.real_raw_logit2, self.real_raw_lin2, self.real_exp_idt2, self.real_exp_logit2, self.real_exp_lin2 = self.G(self.real_exp_idt, self.fake_exp, self.args.mode)

            ########## update D
            self.d_optimizer.zero_grad()
            real_exp_preds = self.D(self.real_exp)                   # 1
            fake_exp_preds = self.D(self.fake_exp_store.detach())    # 0
            d_loss = self.criterionGAN(real_exp_preds, fake_exp_preds, None, None, for_discriminator=True)
            if self.args.adv_input:
                input_preds = self.D(self.real_raw)                  # 0
            ...
            real_exp_preds = self.D(self.real_exp)                   # 1
            fake_exp_preds = self.D(self.fake_exp)                   # 0
            ...
            g_percep_loss += self.args.lambda_percep * self.criterionPercep((self.real_exp_idt+1.)/2., (self.real_exp+1.)/2.)
            self.g_percep_loss = g_percep_loss.item()
            g_loss += g_percep_loss

            # self.real_exp_idt, _, self.real_exp_logit = self.G(self.real_exp)
            # g_idt_loss = self.args.lambda_idt * self.criterionIdt(self.real_exp_idt, self.real_exp)
            # self.g_idt_loss = g_idt_loss.item()
            # g_loss += g_idt_loss
            # g_idt_loss = self.args.lambda_idt * self.criterionIdt(self.fake_exp, self.real_exp)
            # g_idt_loss += self.args.lambda_idt * self.criterionIdt(self.real_exp_idt, self.real_raw)
            g_idt_loss = self.args.lambda_idt * self.criterionIdt(self.real_exp_idt, self.real_exp)
            # g_idt_loss += self.args.lambda_idt * self.criterionIdt(self.real_exp_idt, self.real_raw)
            self.g_idt_loss = g_idt_loss.item()
            g_loss += g_idt_loss

            g_logit_loss = self.criterionClass(self.real_raw_logit, torch.ones_like(self.real_raw_logit)) + self.criterionClass(self.real_exp_logit, torch.zeros_like(self.real_exp_logit))
            g_logit_loss = self.args.lambda_logit * g_logit_loss
            self.g_logit_loss = g_logit_loss.item()
            g_loss += g_logit_loss

            # g_l1_loss = self.L1_loss(self.real_raw, self.fake_exp2)
            # g_l1_loss += self.L1_loss(self.real_raw, self.fake_exp2)
            # g_l1_loss = self.args.lambda_l1 * g_l1_loss
            # self.g_l1_loss = g_l1_loss.item()
            # g_loss += g_l1_loss

            # self.r2e2r, _, self.e2r2e, _ = self.G(self.fake_exp, self.real_exp_idt, self.args.mode)
            # g_cyc_loss = self.args.lambda_cyc * self.L1_loss(self.r2e2r, self.real_raw)
            # g_cyc_loss += self.args.lambda_cyc * self.L1_loss(self.e2r2e, self.real_exp)
            # self.g_cyc_loss = g_cyc_loss.item()
            # g_loss += g_cyc_loss

            # _, self.fake_exp_latent, _ = self.G(self.fake_exp)
            # _, self.real_exp_idt_latent, _ = self.G(self.real_exp_idt)
            # # g_logic_loss = self.criterionLatent(self.real_raw_logit, torch.ones_like(self.real_raw_logit))
            # # g_logic_loss += self.criterionLatent(self.real_exp_logit, torch.zeros_like(self.real_exp_logit))
            # g_latent_loss = self.args.lambda_latent * self.criterionLatent(self.real_raw_latent, self.fake_exp_latent)
            # g_latent_loss += self.args.lambda_latent * self.criterionLatent(self.real_exp_logit, self.real_exp_idt_latent)
            # self.g_latent_loss = g_latent_loss.item()
            # g_loss += g_latent_loss

            # _, _, self.real_raw_lin2, _, _, self.real_exp_lin2 = self.G(self.fake_exp, self.real_exp_idt, self.args.mode)
            # g_lin_loss = self.criterionLin(self.real_raw_lin2, self.real_exp_lin, self.real_raw_lin)
            # g_lin_loss += self.criterionLin(self.real_exp_lin2, self.real_raw_lin, self.real_exp_lin)
            # g_lin_loss = self.args.lambda_lin * g_lin_loss
            # self.g_lin_loss = g_lin_loss.item()
            # g_loss += g_lin_loss
    ...
    def logging(self, step):
        ...
        self.loss['G/logit_loss'] = self.g_logit_loss
        # self.loss['G/l1_loss'] = self.g_l1_loss
        # self.loss['G/latent_loss'] = self.g_latent_loss
        # self.loss['G/cyc_loss'] = self.g_cyc_loss
        # self.loss['G/lin_loss'] = self.g_lin_loss
    ...
    def print_info(self, step, total_steps, pbar):
        ...
        if (step + 1) % self.args.info_step == 0:
            ...
            pbar.write("Elapse:{:>.12s}, D_Step:{:>6d}/{}, G_Step:{:>6d}/{}, D_loss:{:>.4f}, G_loss:{:>.4f}, G_percep_loss:{:>.4f}, G_adv_loss:{:>.4f}, G_idt_loss:{:>.4f}, G_logit_loss:{:>.4f}".format(elapsed, step + 1, total_steps, (step + 1), total_steps, self.d_loss, self.g_loss, self.g_percep_loss, self.g_adv_loss, self.g_idt_loss, self.g_logit_loss))
    ...
    def model_validation(self, step):
        if (step + 1) > self.val_start_steps:
            if (step + 1) % self.val_each_steps == 0:
                ...
                val_save_path_gt = self.val_result_path + '/' + 'validation_gt'
                ...
                if not os.path.exists(val_save_path_gt):
                    os.makedirs(val_save_path_gt)
                ...
                with torch.no_grad():
                    for val_step in range(val_start, val_total_steps):
                        ...
                        val_label, val_real_raw, val_name = input.img_exp, input.img_raw, input.img_name
                        self.mode = 'test'
                        val_fake_exp, _, _, _, _, _ = self.G(val_real_raw, None, self.mode)

                        for i in range(0, denorm(val_real_raw.data).size(0)):
                            save_imgs = denorm(val_fake_exp.data)[i:i + 1,:,:,:]
                            save_image(save_imgs, os.path.join(val_save_path, '{:s}.png'.format(val_name[i])))
                            save_imgs_gt = denorm(val_label.data)[i:i + 1, :, :, :]
                            save_image(save_imgs_gt, os.path.join(val_save_path_gt, '{:s}.png'.format(val_name[i])))
                        ...
                        val['val/vallabel'] = denorm(val_label.detach().cpu())
                    ...
                    if self.args.is_test_psnr_ssim:
                        ...
                        curr_psnr = calc_psnr(val_save_path, val_save_path_gt, self.psnr_save_path, current_epoch)
                        ...
                        curr_ssim = calc_ssim(val_save_path, val_save_path_gt, self.ssim_save_path, current_epoch)
