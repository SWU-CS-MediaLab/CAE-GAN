...
class PerceptualLoss(nn.Module):
    def __init__(self):
        ...
        # view()改变tensor的形状，-1是不确定的，让电脑计算
        # 所以这里的mean和std应该是一长条的结构
    ...
    def __call__(self, x, y):
        # x的形状是[1, 3, 256, 256]
        # 如果x的第二个维度不是3，就让x和y在第二维度上重复3次(channel)
        if x.shape[1] != 3:
            x = x.repeat(1, 3, 1, 1)
            y = y.repeat(1, 3, 1, 1)
        # 归一化
        ...
        # 前四个权重很小，第五个权重最大
        # x_vgg['relu1_1']：把x放入vgg后的第relu1_1的输出
    ...
    def get_target_tensor(self, input, target_is_real):
        if target_is_real:
            if self.real_label_tensor is None:
                #  real_label_tensor:[1.0]
                self.real_label_tensor = self.Tensor(1).fill_(self.real_label)
                self.real_label_tensor.requires_grad_(False)
            # 将real_label_tensor扩充的和input一样大
    ...
    def loss(self, real_preds, fake_preds, target_is_real, for_real=None, for_fake=None, for_discriminator=True):
        if self.gan_mode == 'original': # cross entropy loss
            if for_real:
                ...
                # F.binary_cross_entropy_with_logits()是nn.sigmoid和nn.BCELoss的合体
