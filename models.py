...
class Lap_Pyramid_Con(nn.Module):
    def __init__(self, num_high=3):
        super(Lap_Pyramid_Con, self).__init__()
...
class Generator(nn.Module):
    ...
    def __init__(self, conv_dim, norm_fun, act_fun, use_sn, num_high=3, nrb_low=5):
        super(Generator, self).__init__()
        self.relu = nn.ReLU(inplace=False)
        # encoder
        # conv_dim=32
        # 256*256*3 --> 256*256*32
        self.lap_pyramid = Lap_Pyramid_Con(num_high)
        self.enc11 = ConvBlock(in_channels=4, out_channels=conv_dim * 1, kernel_size=7, stride=1, padding=0, dilation=1,
        ...
        # self.enc5 = nn.Conv2d(in_channels=conv_dim * 8, out_channels=conv_dim * 16, kernel_size=1, bias=False)
        ...
        self.dec2 = ConvBlock(in_channels=conv_dim * 8 + 3, out_channels=conv_dim * 4, kernel_size=3, stride=1, padding=0,
                              dilation=1, use_bias=True, norm_fun='ILN', act_fun=act_fun, use_sn=use_sn)

        self.dec3 = ConvBlock(in_channels=conv_dim * 4 + 3, out_channels=conv_dim * 2, kernel_size=3, stride=1, padding=0,
                              dilation=1, use_bias=True, norm_fun='ILN', act_fun=act_fun, use_sn=use_sn)
        ...
        self.dec5 = nn.Sequential(
            SNConv(in_channels=conv_dim * 1 + 3, out_channels=conv_dim * 1, kernel_size=3, stride=1, padding=0, dilation=1,
        ...
        self.conv_lap1 = nn.Conv2d(3, 256, 3, padding=1)
        self.conv_lap2 = nn.Conv2d(3, 128, 3, padding=1)
        self.conv_lap3 = nn.Conv2d(3, 64, 3, padding=1)
        self.conv_lap4 = nn.Conv2d(3, 32, 3, padding=1)

    def forward(self, raw, exp, mode):
        if mode == 'train':
            A_img = torch.squeeze(raw, 0) 
            r, g, b = A_img[0] + 1, A_img[1] + 1, A_img[2] + 1
            A_gray = 1. - (0.299 * r + 0.587 * g + 0.114 * b) / 2. 
            A_gray = torch.unsqueeze(A_gray, 0) 
            A_gray = torch.unsqueeze(A_gray, 0)

            gray_2 = self.downsample_1(A_gray) 
            gray_3 = self.downsample_2(gray_2) 
            gray_4 = self.downsample_3(gray_3) 
            pyr_A = self.lap_pyramid.pyramid_decom(img=raw)
            pyr_B = self.lap_pyramid.pyramid_decom(img=exp)

            x = torch.cat((raw, A_gray), 1)
            raw1 = self.enc11(x)  
            raw2 = self.enc2(raw1)  
            raw2 = raw2 * gray_2 + raw2
            raw3 = self.enc3(raw2)  
            raw3 = raw3 * gray_3 + raw3
            raw4 = self.enc4(raw3) 
            raw4 = raw4 * gray_4 + raw4
            ...
            raw_out = raw4 * raw_out_z  # [2, 256, 32, 32]
            ...
            raw_y2 = self.upsample2(raw_out) 

            raw_y2 = torch.cat([raw_y2, raw3, pyr_A[2]], dim=1) 
            raw_y2 = self.dec2(raw_y2)

            raw_y3 = self.upsample3(raw_y2) 
            raw_y3 = torch.cat([raw_y3, raw2, pyr_A[1]], dim=1) 
            raw_y3 = self.dec3(raw_y3)  

            raw_y4 = self.upsample4(raw_y3)  
            ...
            raw_fout = raw_y4 * raw_x1_out 
            raw_fout = torch.cat([raw_fout, pyr_A[0]], dim=1)
            raw_fout = self.dec5(raw_fout)  

            raw_fout = raw_fout * A_gray

            raw_fout = torch.clamp((raw_fout + raw), min=-1.0, max=1.0)

            exp_y2 = self.upsample2(exp_out) 
            exp_y2 = torch.cat([exp_y2, exp3, pyr_B[2]], dim=1) 
            exp_y2 = self.dec2(exp_y2) 

            exp_y3 = self.upsample3(exp_y2) 
            exp_y3 = torch.cat([exp_y3, exp2, pyr_B[1]], dim=1) 
            exp_y3 = self.dec3(exp_y3)

            exp_y4 = self.upsample4(exp_y3) 
            ...
            exp_fout = exp_y4 * exp_x1_out  
            exp_fout = torch.cat([exp_fout, pyr_B[0]], dim=1)  
            exp_fout = self.dec5(exp_fout) 

            exp_fout = exp_fout * A_gray
...
class Discriminator(nn.Module):
    def __init__(self, conv_dim, norm_fun, act_fun, use_sn, adv_loss_type):
        ...
        self.mean_mlp1 = MLP(ch=32, n_mix=2)
        self.stddev_mlp1 = MLP(ch=32, n_mix=2)
        self.max_mlp1 = MLP(ch=32, n_mix=2)
        self.mean_mlp3 = MLP(ch=128, n_mix=2)
        self.stddev_mlp3 = MLP(ch=128, n_mix=2)
        self.max_mlp3 = MLP(ch=128, n_mix=2)
        self.mean_mlp5 = MLP(ch=512, n_mix=2)
        self.stddev_mlp5 = MLP(ch=512, n_mix=2)
        self.max_mlp5 = MLP(ch=512, n_mix=2)

    def forward(self, x):
        ds1 = self.d1(x) 
        ds1_pred = self.d1_pred(ds1) 

        gap1 = F.adaptive_avg_pool2d(ds1, 1)  
        gap1 = gap1.squeeze(3).squeeze(2)
        gap_logit1 = self.mean_mlp1(gap1) 
        gap_logit1 = gap_logit1.unsqueeze(2)
        gap_logit1 = gap_logit1.unsqueeze(3)

        gmp1 = F.adaptive_max_pool2d(ds1, 1)  
        gmp1 = gmp1.squeeze(3).squeeze(2) 
        gmp_logit1 = self.max_mlp1(gmp1) 
        gmp_logit1 = gmp_logit1.unsqueeze(2)
        gmp_logit1 = gmp_logit1.unsqueeze(3)
        ...
        gap3 = F.adaptive_avg_pool2d(ds3, 1)  
        gap3 = gap3.squeeze(3).squeeze(2) 
        gap_logit3 = self.mean_mlp3(gap3) 
        gap_logit3 = gap_logit3.unsqueeze(2)
        gap_logit3 = gap_logit3.unsqueeze(3)

        gmp3 = F.adaptive_max_pool2d(ds3, 1) 
        gmp3 = gmp3.squeeze(3).squeeze(2) 
        gmp_logit3 = self.max_mlp3(gmp3) 
        gmp_logit3 = gmp_logit3.unsqueeze(2)
        gmp_logit3 = gmp_logit3.unsqueeze(3)
        ...
        gap5 = F.adaptive_avg_pool2d(ds5, 1)  
        gap5 = gap5.squeeze(3).squeeze(2) 
        gap_logit5 = self.mean_mlp5(gap5) 
        gap_logit5 = gap_logit5.unsqueeze(2)
        gap_logit5 = gap_logit5.unsqueeze(3)

        gmp5 = F.adaptive_max_pool2d(ds5, 1)  
        gmp5 = gmp5.squeeze(3).squeeze(2)
        gmp_logit5 = self.max_mlp5(gmp5)  
        gmp_logit5 = gmp_logit5.unsqueeze(2)
        gmp_logit5 = gmp_logit5.unsqueeze(3)

        return [ds1_pred, ds3_pred, ds5_pred, ds3_three_pred, gap_logit1, gap_logit3, gap_logit5, gmp_logit1, gmp_logit3, gmp_logit5


class MLP(nn.Module):

    def __init__(self, ch, n_mix=2):
        super().__init__()
        self._ch = ch
        self._n_mix = n_mix
        net = []
