...
class Tester(object):
    ...
    def test(self):
        ...
        test_save_path_gt = self.test_result_path + '/' + 'test_results_gt'
        test_compare_save_path = self.test_result_path + '/' + 'test_compare'

        if not os.path.exists(test_save_path):
            os.makedirs(test_save_path)
        if not os.path.exists(test_save_path_gt):
            os.makedirs(test_save_path_gt)
        ...
        with torch.no_grad():
            for test_step in range(test_start, test_total_steps):
                ...
                test_label, test_real_raw, test_name = input.img_exp, input.img_raw, input.img_name

                test_fake_exp, _, _, _, _, _ = self.G(test_real_raw, None, self.args.mode)

                for i in range(0, denorm(test_real_raw.data).size(0)):
                    save_imgs = denorm(test_fake_exp.data)[i:i + 1,:,:,:]
                    save_image(save_imgs, os.path.join(test_save_path, '{:s}.png'.format(test_name[i])))
                    save_imgs_gt = denorm(test_label.data)[i:i + 1, :, :, :]
                    save_image(save_imgs_gt, os.path.join(test_save_path_gt, '{:s}.png'.format(test_name[i])))
                ...
                test['test/testlabel'] = denorm(test_label.detach().cpu())
            ...
            if self.args.is_test_psnr_ssim:
                ...
                curr_psnr = calc_psnr(test_save_path, test_save_path_gt, self.psnr_save_path, self.args.pretrained_model)
                print("====== Avg. PSNR: {:>.4f} dB ======".format(curr_psnr))

                self.ssim_save_path = './results/ssim_test_results/' 
                curr_ssim = calc_ssim(test_save_path, test_save_path_gt, self.ssim_save_path, self.args.pretrained_model)
